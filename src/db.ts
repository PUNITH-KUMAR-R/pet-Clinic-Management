...
const DATA_DIR = isLambda ? path.join('/tmp', 'data') : SEED_DATA_DIR;
  ...
  private ensureDatabaseFile() {
    try {
      ...
      if (!fs.existsSync(DB_FILE)) {
        const seedFile = path.join(SEED_DATA_DIR, 'clinic_db.json');
  ...
  public readTrash(): TrashItem[] {
    try {
      if (!fs.existsSync(TRASH_FILE)) {
        ...
        const seedTrash = path.join(SEED_DATA_DIR, 'trash.json');
        if (fs.existsSync(seedTrash)) {
          const content = fs.readFileSync(seedTrash, 'utf-8');
          fs.writeFileSync(TRASH_FILE, content, 'utf-8');
          return JSON.parse(content) as TrashItem[];
        }
